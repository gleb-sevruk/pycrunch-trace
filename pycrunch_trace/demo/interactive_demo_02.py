...
def method_in_another_file(some_number: int):
    ...
    print('You can see on the graph that we are at 300ms mark of the recording.')
    print('Current time (since recording start) is also displayed in inspector')

    print('You can skip next method call by using step over (Down Arrow)')
...
def dummy_method():
    print('Notice `index` variable as you step through the loop')
